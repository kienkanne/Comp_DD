...
@contextmanager
def gnu_parallel(cmds, n_jobs=1, title="", skip=False):
    """
    Warning: From update 2.3.0, 'gnu_parallel' is depreciated.
    'Parallel' is no longer listed as a requirement in environment.yaml
    """

    # --- Everything BEFORE the yield runs before the execution ---
    scratch_tmp = Path(tempfile.gettempdir()).expanduser()
    os.makedirs(scratch_tmp, exist_ok=True)
    
    processed_cmds = [shlex.join([os.path.expandvars(str(t)) for t in c]) for c in cmds]
    stdin = "\n".join(processed_cmds)
    
    ctx = PipelineContext.get_ctx()
    logger = ctx.logger if ctx.logger is not None else DummyLogger()
    logger.info(f"Running: Parallel {title} for {len(cmds)} jobs")
    ...
    try:
        ...
        logger.info(f"Parallel execution completed. Kept {successful_jobs}/{total_jobs} successful jobs.")
        yield outputs
